from data_struct import GameState, Piece, PieceKind, Player, Move, BOARD_COLS, BOARD_ROWS, grid_get, is_valid_pos
...
def move_to_str(move: Move, piece: Piece) -> str:
    """Transforme (src: Pos, dst: Pos, kind: MoveKind) -> 'MoveKind [WB] (A4->C5)'"""
    cols = "ABCDEFGHI"
    color = "W" if piece.player == Player.WHITE else "B"
    src = f"{cols[move.src[1]]}{move.src[0] + 1}"
    dst = f"{cols[move.dst[1]]}{move.dst[0] + 1}"
    return f"{move.kind.value} [{color}{sym[piece.kind]}] ({src}->{dst})"